...
def _html(media_url: str, thumb_url: str | None, sprite_url: str | None,
    ...
    # ── OG video (только для видео, без width/height чтобы не показывало размер) ──
    og_video = ""
    if kind != "photo":
        og_video  = f'  <meta property="og:video"            content="{stream_url}"/>\n'
        og_video += f'  <meta property="og:video:secure_url" content="{stream_url}"/>\n'
        og_video += f'  <meta property="og:video:type"       content="video/mp4"/>'
        if duration:
...
def _success_message(kind: str, url: str) -> str:
    """Как во вставленном боте: жирный заголовок + ссылка «Открыть» (HTML)."""
    titles = {
        "photo": "Фото готово",
        "video": "Видео готово",
